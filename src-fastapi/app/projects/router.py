...
async def create_project(project: ProjectCreate, service: ProjectServiceDependency) -> ProjectRead:
...
async def read_project(project_id: int, service: ProjectServiceDependency) -> ProjectRead:
...
async def replace_project(
    project_id: int, project: ProjectReplace, service: ProjectServiceDependency
) -> ProjectRead:
...
async def update_project(
    project_id: int, project: ProjectUpdate, service: ProjectServiceDependency
) -> ProjectRead:
    return ProjectRead.model_validate(await service.update(project_id, project))


@router.delete("/{project_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_project(project_id: int, service: ProjectServiceDependency) -> None:
